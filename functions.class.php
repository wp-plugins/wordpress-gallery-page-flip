...
class Functions
{
   ...
   //вспомогательная функция создание каталога
	function createDir( $dirName, $permit = 0777 )
	{
		...
		// создаем папку если нету её ещё
		if ( !is_dir( $dirName ) )
		{
		...
		//проверяем свободна ли для записи директория
		...
		{
			// для safe_mode , лучше удалить папку и пусть вручную создают
			rmdir( $dirName );
	...
	//функция определяющая размеры фото
	...
	//функция для вывода на печать превьюшки
	function printImg( $img, $alt = '', $width = '', $height = '', $pageList = false )
	{
	    ...
	    /*$fileExt = split( "\.", $pathParts['basename'] );*/
	    ...
	    switch( strtolower( $fileExt[1] ) )
	    {
	    	...
	    	default :
	    	{
				...
				//if( $imageSize = @getimagesize( $thumb ) ) $img = $thumb;
		        //else $imageSize = @getimagesize( $img );
		        //--
		        //$img = $thumb;
		        ...
		        if( !ini_get( 'allow_url_fopen' ) || ini_get( 'allow_url_fopen' ) == 'Off' )
					$photoSize = array( 'width' => $width,
										'height' => $height );
		        else /*if (file_exists($thumb) || get_headers($thumb))*/
		        {
					$imageSize = $this->getImageSize( $thumb );
					if( !$imageSize ) return __('No image', 'pageFlip');  //если нет изображения
	...
	//функция для создания превьюшек
	...
	//функция чтобы вытащить имя исходного файла из порезанной части
	...
	//перегружаем функцию getimagesize
	...
	//получение полного изобюражения по превьюшке
	...
	//функция получения расширения
	function getExt( $filename )
	{
		//получаем расширение файла
	    /*$fileExt = split( "\.", basename( $filename ) );*/
	...
	{
	    //$fileExt = split( "\.", basename( $oldName ) );
  ...
  function checkImage( $imageName )
  {
    ...
    $formats = array( 'jpg', 'jpeg', 'gif', 'swf' );
	...
	function checkTable( $tableName, $fields )
    {
		...
		foreach( $fields as $field=>$prop )
		 if( !in_array( $field, $existsFields ) ) //если такого поля нет - добавляем
   ...
   function navigationBar( $page = '', $imgPerPage = '', $type = 'pageFlip', $adParam = '', $gallery = 0 )
   {
		...
		$countNumbers = 7; //количество отображаемых страниц
		...
		$countPages = ceil( $countImg / $imgPerPage ); //количество страниц
   ...
   //функция для формирования списка выбора галерей
   ...
   //функция, которая формирует по скольку показывать изображений
   ...
   //функция с sql запросами для списка изображений
   ...
   //функция, возвращающая превьюшку галлереи
   ...
   //функция формирования xml для загрузки альбома в флешедитор
...
/*   		$width = ceil( $book->width / 2 );

		$images = ''; $pages = '';
		foreach( $book->pages as $page )
   		{
   			if( $this->getExt( $page->image ) == 'swf' ) continue;

			$size = $this->getImageSize( $page->image );
   			$thumb = $this->getThumb( $page->image );
			$sizeThumb = $this->getImageSize( $thumb );

			$images .= '<img id="' . $page->number . '" orig="' . $page->image . '" thumb="' . $thumb . '" width="' . $size[0] . '" height="' . $size[1] . '" thumbwidth="' . $sizeThumb[0] . '" thumbheight="' . $sizeThumb[1] . '"  />' . "\n";
   			$pages .= '';
   		}

   		return <<<XML
<?xml version="1.0" encoding="UTF-8"?>
<album id="{$book->id}" newAlbum="true" name="Album" type="landscape" pageWidth="{$width}" pageHeight="{$book->height}">
<!-- backgroundColor="0xdadfe2" frameColor="0xFFFFFF" frameSize="14"-->
  <images>
    {$images}
  </images>
</album>
		...
		if( $page->modified == 'true' )
	 	{
			...
			if( $newPage )
			{
				...
				//if( $this->getExt( $oldPage ) == "swf" && count( $page->imgs ) == 0 )

				//если раньше тоже была картинка из флешэдитора
				//то удаляем картинку нах
   ...
   function warningMessage( $message )
   {
	...
	function imgWriteText( $imageFile, $text, $style = array('left'=>0, 'top'=>0, 'fontFamily'=>'Helvetica', 'fontSize'=>14, 'color'=>'000000'), $rename = false )
	{
		...
		if ($rename)
		{
			...
			//unlink( $path['dirname'].'/'. 't_'.basename($imageFile) );
